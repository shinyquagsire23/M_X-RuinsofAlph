...
void boxPrint(u8 windowId, u8 font, u8 x, u8 y, u32 bar, u32 baz, u32 txtpointer) // AddTextPrinterParameterized3
{
	int (*func3)(u8,u8,u8,u8,u32,u32,u32) = (int (*)(void))0x08199E65;
	func3(windowId,font,x,y,bar,baz,txtpointer);
}

void writeBoxesToTilemap(u8 windowId, u8 mode) // CopyWindowToVram
{
	int (*func5)(u8,u8) = (int (*)(void))0x08003659;
	func5(windowId,mode);
}

void drawTutorialBar(u8 windowId, u8 value) // FillWindowPixelBuffer
{
...
u32 loadTutorialText(u32 *textAddr)
{
	...
	u16 (*AddWindow)(const struct WindowTemplate *) = (void *)0x8003381;
	u8 windowId = AddWindow(&templ);
	int (*func)(u8) = (int (*)(u32))0x08098C19;		/* GetTextWindowPalette */   //load tutorial bar palette
	loadPalette(func(2),0xB0,0x20); // LoadPalette(src, slot, size)
	drawTutorialBar(windowId,0xFF);
	boxPrint(windowId,0x0,0x2,0x1,instsData,0x00000000,textAddr);
	
	int (*func4)(u8) = (int (*)(void))0x0800378D;
	func4(windowId); // PutWindowTilemap(windowId)
